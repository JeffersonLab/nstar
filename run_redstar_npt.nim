...
## ----------------------------------------------------------------------------
proc run_redstar_npt*(arch: string; stem, chan, irrep: string, seqno: string) =
  let exes = redstar_exe(arch)             # Setup the executables
...
when isMainModule:
  echo "Run redstar"
  run_redstar_npt("nstar", "test", "rho", "000_T1mP", "1")
