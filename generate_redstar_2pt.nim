...
type
  HadronIrrepOpInfo_t* = object ## Given an IrrepOp struct, return useful information about the construction
    ## This struct holds info about the entire N-body construction, so not the same as in HadronOpsRegInfo
    N*:                cint           ##
...
proc hadronIrrepOpInfo*(irrep_op: KeyHadronSUNNPartIrrepOp_t;
                        irrepPG: string;
                        flavor: KeyCGCSU3_t;
                        irmom: KeyCGCIrrepMom_t): HadronIrrepOpInfo_t =
  return HadronIrrepOpInfo_t(N: 2, F: "3", irrepPG: irmom, dim: 1, mom_type: Mom3d(0,0,0),
...
proc printRedstarIrrep*(ops_list: seq[OpsList_t];
  ...
  let canon_mom = canonicalOrder(irmom.mom)
  #  Sink ops
  result.setLen(0)
...
proc printRedstar2PtsDiag*(source_ops_list: seq[OpsList_t];
  ...
  let source_ops = printRedstarIrrep(source_ops_list, ops_map, flavor, irmom, include_all_rows, true, true)
  #  Output
  result.setLen(0)
...
proc printRedstar2PtsOffDiag*(source_ops_list: seq[OpsList_t];
  ...
  ## Off-diagonal method
  #  Sink & source ops
  let sink_ops   = printRedstarIrrep(sink_ops_list, ops_map, flavor, irmom, include_all_rows, false, true)
  let source_ops = printRedstarIrrep(source_ops_list, ops_map, flavor, irmom, include_all_rows, true, true)
  #  Output
  result.setLen(0)
